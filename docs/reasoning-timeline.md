...
- The process-local default is `1h`. Clicking the merged `TURN · 1h` control opens a vertically snapping macOS-style wheel for `1h`, `3h`, `6h`, `12h`, and `24h`; its centered value is emphasized while values toward the top and bottom fade. Expanding the window raises the Thread discovery limit proportionally and reads more rollout history; at most the latest 240 visible Turns are rendered.
...
- Disk detail records expire after seven days and the directory is capped at 128MB. Expanding the history wheel to an earlier cutoff reparses unchanged rollout files once when the earlier detail range has not yet been cached.

...
![CodexIQ — local Codex reasoning intelligence](Resources/codexiq-hero.png)
...
CodexIQ turns local Codex telemetry into a real-time map of reasoning behavior. Each Thread gets a stable color and each Turn becomes a node: vertical position is total reasoning tokens, node area is total token volume, and small status accents flag invalid, unknown, or running work without hiding Thread identity.

The menu bar stays intentionally quiet with one template icon. The first click opens the full timeline and Turn inspector directly in one native popover. All rollout and token data stays on your Mac.

CodexIQ is the product-facing name introduced after `v1.0.1`. The repository, Homebrew token, bundle identifier, executable, cache path, archive naming, and transitional `Codex Pulse.app` bundle filename remain unchanged so existing installations can upgrade without losing preferences or caches.
...
- **Model-call distribution.** Click a Turn to reveal its internal reasoning-call chart with a linear token axis, red issue points, count, range, median, duration, model, effort, and rollout link.
...
- **Longer history on demand.** Switch the in-panel `1h`, `3h`, `6h`, `12h`, and `24h` segmented control only when you need more history.

## Interaction

| Action | Result |
| --- | --- |
| Click the menu-bar icon | Open the full CodexIQ timeline and inspector |
...
| Escape | Close the Dashboard popover |

## Local-first by design

CodexIQ talks to the Codex app-server already available on your Mac and reads rollout JSONL from local paths returned by `thread/list`. It does not run a cloud service, add analytics, upload prompts, or copy rollout contents outside your machine.
...
- Swift Charts are created only while the Dashboard popover is open and released when it closes.
...
CodexIQ prefers the embedded Desktop CLI so its app-server protocol matches the running Codex application. Set `CODEX_APP_SERVER_EXECUTABLE` to override discovery when developing against another build.
...
```bash
./scripts/package_release.sh --local 1.0.1
```

`--local` always produces an ad-hoc-signed development candidate. It cannot be
mistaken for a public release. Public packaging is fail-closed until a Developer
ID Application identity and a notarization profile are available; see the
[release contract](docs/release.md).

The menu-bar icon opens one screen-constrained native AppKit popover, with a preferred 780×560 SwiftUI + Swift Charts content area. Swift actors retain app-server, rollout, and cache isolation.
